...
//MARK::- VIEW EXTENSION
//MARK::- CUSTOM ANIMATION FOR ICONS

extension UIView {

    func addBorderOnViewBottom(size: CGFloat, color: UIColor)->CALayer {
        return addBorderViewUtility(x: 0, y: frame.height - size, width: frame.width, height: size, color: color)
    }

    fileprivate func addBorderViewUtility(x: CGFloat, y: CGFloat, width: CGFloat, height: CGFloat, color: UIColor)->CALayer {
        let border = CALayer()
...
extension UIView{
    func customAnimate(){
        ...
        UIView.animate(withDuration: 0.2, animations: {
            self.alpha = 1
            self.transform = CGAffineTransform.init(scaleX: 1.3, y: 1.3).concatenating(CGAffineTransform.init(rotationAngle: -0.5))
        }) { (val) in
            if val{
                UIView.animate(withDuration: 0.2, animations: {
                    self.transform = CGAffineTransform.init(scaleX: 1, y: 1).concatenating(CGAffineTransform.init(rotationAngle: 0))
                }, completion: nil)
...
//MARK::- SPRING ANIMATION FOR ICONS

extension UIView{
    func springAnnimate() {
        self.transform = CGAffineTransform.init(translationX: 0, y: -8)
        UIView.animate(withDuration: 2.0,
                       delay: 0,
                       usingSpringWithDamping: CGFloat(0.20),
                       initialSpringVelocity: CGFloat(2.0),
                       options: [UIView.AnimationOptions.allowUserInteraction , .repeat],
                       animations: {
...
//MARK:- the amount of spacing to appear between image and title
extension UIButton {
    func spaceBWimageAndTitle(spacing:CGFloat){
        imageEdgeInsets = UIEdgeInsets(top: 0, left: spacing, bottom: 0,right: 0)
    }
    
    func spaceFortitle(spacing:CGFloat){
...
extension UIView {
    ...
    func drawShadow(shadowColor:UIColor = #colorLiteral(red: 0.2549019754, green: 0.2745098174, blue: 0.3019607961, alpha: 0.798828125), shadowOpacity:Float = 0.14, shadowPath:UIBezierPath? = nil, shadowRadius:Float = 6, cornerRadius:Float = 6.0, offset: CGSize = CGSize(width: 0, height: 0)) {
