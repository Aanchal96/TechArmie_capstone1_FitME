...
class DownloadController {
    ...
    func saveAudio(urls: [String], ids: [String], completed : @escaping ([URL]) -> Void, progress: @escaping (Int) -> Void , failure : @escaping (Error) -> Void) {
        ...
        urls.forEachEnumerated({[weak self] (offset, element) in
            ...
            if !((videoUrl.description.contains(".mp3")) || (videoUrl.description.contains(".MP3")) || (videoUrl.description.contains(".wav")) ){
                dispatch.leave()
                isFailed = true
                failure((NSError.init(localizedDescription: "invalid error") as Error))
                return
            ...
            let fileProp : (Bool,URL?) = strongSelf.checkIfFileExists(ids[offset] , isAudio : true)
            if fileProp.0 {
                if let url = fileProp.1 {
                    localUrls.append(url)
                    progressCount += 1
            ...
            strongSelf.downloadVideo(isAudio : true ,URLString: element, id: ids[offset], mediaType : "audio/mp3", success: { (url) in
                if isFailed{
                    dispatch.leave()
                    return
                }
